        ...
        if (errorArray.length) {
          const errorContainer = new ContainerBuilder()
            .addTextDisplay(
              new TextDisplayBuilder()
                .setDescription(
                  `<:crossmark:1423012536353292380> There was an error when adding user to blacklist.\nError(s):\n ${errorArray.join(
                    `\n`
                  )}`
                )
            );
          await interaction.reply({ components: [errorContainer], flags: MessageFlags.IsComponentsV2, ephemeral: true });
          return;
        } else {
          ...
          collector.on("collect", async (i) => {
            if (i.customId === "remove") {
              ...
              await i.update({ components: [removeContainer], flags: MessageFlags.IsComponentsV2, components2: [] });
            }
          });
