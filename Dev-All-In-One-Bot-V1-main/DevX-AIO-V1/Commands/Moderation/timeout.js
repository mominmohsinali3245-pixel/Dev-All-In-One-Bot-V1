...
module.exports = {
  ...
  async execute(interaction, message, client) {
    ...
    dmContainer.addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`You have been timed out in ${interaction.guild.name}. You can check the status of your timeout within the server. | ${reason}`)
    );

    const container = new ContainerBuilder();
    container.addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`${timeUser.tag} has been **timed out** for ${duration / 60} minute(s) | ${reason}`)
    );
