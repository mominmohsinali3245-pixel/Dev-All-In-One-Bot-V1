    ...
    switch (sub) {
      case "add":
        ...
        addContainer.addTextDisplayComponents(
          new TextDisplayBuilder().setContent(`<:tickmark:1423012532104204449> Successfully added the role: ${role} to ${username}`)
        );

        await member.roles.add(role).catch((err) => {
          addContainer.addTextDisplayComponents(
            new TextDisplayBuilder().setContent(`# Error!\n\n**• Error Occured**\n> Error received trying to add a role to ${username}. **Check** my role position and **permissions** and try again!\n\n💳 Role not Added`)
          );
          return;
        });

        await interaction.reply({ components: [addContainer], flags: MessageFlags.IsComponentsV2 });

        break;
      case "remove":
        const removeContainer = new ContainerBuilder();
        removeContainer.addTextDisplayComponents(
          new TextDisplayBuilder().setContent(`<:tickmark:1423012532104204449> Successfully removed the role: ${role} from ${username}`)
        );

        await member.roles.remove(role).catch((err) => {
          removeContainer.addTextDisplayComponents(
            new TextDisplayBuilder().setContent(`# Error!\n\n**• Error Occured**\n> Error received trying to remove a role from ${username}. **Check** my role position and **permissions** and try again!\n\n💳 Role not Removed`)
          );
          return;
        });

        await interaction.reply({ components: [removeContainer], flags: MessageFlags.IsComponentsV2 });
      ...
      case "members":
        ...
        container.addTextDisplayComponents(
          new TextDisplayBuilder().setContent(`# 📃 Role Members tool\n\n> Role's Members`)
        );
        ...
        if (!memberslist) {
          container.addTextDisplayComponents(
            new TextDisplayBuilder().setContent(`**• Role Members List**\n> No users found with specified role!`)
          );
        } else {
          container.addTextDisplayComponents(
            new TextDisplayBuilder().setContent(`**• Role Members List**\n> ${memberslist.slice(0, 900)}`)
          );
        }

        container.addSeparatorComponents(new SeparatorBuilder());
        container.addTextDisplayComponents(
          new TextDisplayBuilder().setContent(`📃 Role Members fetched`)
        );
      ...
      case "delete":
        ...
        if (!guildrole)
          ...
          deleteContainer.addTextDisplayComponents(
            new TextDisplayBuilder().setContent(`# 💳 Role Tool\n\n> Role Deleted Successfully`)
          );
          deleteContainer.addSeparatorComponents(new SeparatorBuilder());
          deleteContainer.addTextDisplayComponents(
            new TextDisplayBuilder().setContent(`**• Role ID:** ${guildrole.id}\n**• Executer:** ${interaction.user}`)
          );
          deleteContainer.addSeparatorComponents(new SeparatorBuilder());
          deleteContainer.addTextDisplayComponents(
          ...
          await interaction.reply({ components: [deleteContainer], flags: MessageFlags.IsComponentsV2 });
        }
      ...
      case "create":
        ...
        createContainer.addSeparatorComponents(new SeparatorBuilder());
        createContainer.addTextDisplayComponents(
          new TextDisplayBuilder().setContent(`**• Role:** ${newrole}\n**• Role Color:** ${color}\n**• Display Separately:** ${displayside}\n**• Permissions:** ${permissionsarray.join(", ").slice(0, 1000)}`)
        );
        createContainer.addSeparatorComponents(new SeparatorBuilder());
        createContainer.addTextDisplayComponents(
        ...
        await interaction.editReply({ components: [createContainer], flags: MessageFlags.IsComponentsV2 });
    }
  },
};
